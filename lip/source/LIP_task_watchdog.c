...
void watchdog_task( void * pvParameters )
{
    ...
    for (;;) {
        ...
        if( READ_ZERO_POSITION_REACHED )
        {
            ...
            // Set output voltage to zero again in case any controller 
            // task managed to set any output voltage
            dcm_set_output_volatage( 0.0f );
            ...
            // UPC or DPC controller was on, this means that app was 
            // already initialized (in default state). Change app state 
            // back to default
            app_current_state = DEFAULT;
        }
        ...
        if (app_current_state == SWINGUP) {
            ...
            if ((ang_error < 126.0f*PI/180.0f) && 
                (ang_error > 0.0f)) {
                // Pendulum angle error is within pm. 25 degrees from 
                // up position
