...
void ctrl_3_FSF_downpos_task( void *pvParameters )
{
    ...
    float switch_angle_low  = (180 - 80) * PI/180.0f; // lower boundry in rad
    float switch_angle_high = (180 + 80) * PI/180.0f; // upper boundry in rad
    ...
    for ( ;; ) {
        uint8_t is_pend_angle_ok = 
            switch_angle_low < pendulum_angle_in_base_range_dpc &&
            switch_angle_high > pendulum_angle_in_base_range_dpc;

        if ( is_pend_angle_ok ) {
            // Controller should only work when pendulum arm angle is 
            // in range [switch_angle_low, switch_angle_high]

            // Calculate state errors
            cart_position_error = *cart_position_setpoint_cm - cart_position[0];
            cart_speed_error    = - cart_speed[ 0 ];
            pend_position_error = 
                pendulum_arm_angle_setpoint_rad_dpc - pend_angle[ 0 ];
            pend_speed_error    = - pend_speed[ 0 ];

            // Calculate control signal contribution of each state variable 
            // error, non linear cart position gain. When cart postion error 
            // is >0 linear feedback with offset +1V is used to compensate for
            // voltage deadzone, for <0 error, y-axis mirror is used 
            ...
            if (cart_position_error > cart_position_allowed_error_cm) {
                // ctrl_cart_position_error = 
                //     tanhf( 8.0f * cart_position_error ) * 
                //     ( gains[0] * cart_position_error + voltage_deadzone );
                ctrl_cart_position_error = 
                    gains[0] * cart_position_error + voltage_deadzone;
            ...
            uint8_t pend_pos_error_in_allowed = 
                pend_position_error < pend_position_allowed_error && 
                pend_position_error > -pend_position_allowed_error;

            if (pend_pos_error_in_allowed) {
            ...
            // if( cart_current_zone == DANGER_ZONE_L ||
            //     cart_current_zone == DANGER_ZONE_R ) {
            //     ctrl_signal /= 3;
            // }
            // if( ( cart_position[0] < 5.0f ) || 
            //     ( cart_position[0] > (TRACK_LEN_MAX_CM-5.0f) ) )
