...
void cart_worker_task( void * pvParameters )
{
    ...
    {   
        ...
        xTaskNotifyWaitIndexed(0, 0x00, ULONG_MAX, &notif_value_received,
                               portMAX_DELAY);
                
        // notif_value_received possible values: GO_RIGHT, GO_RIGHT+GO_LEFT, 
        // GO_LEFT, SP_HOME cart_position_calibrated
        if ( notif_value_received == GO_RIGHT ) {
            // app is uninitialized state and cart is already in zero position 
            // OR app is in default state (cart position calibrated). Move cart 
            // to the right until track center reached
            ...
            // change app state from UNINITIALIZED to DEFAULT right after zero
            // position is reached
            ...
            // app is in uninitialized state and cart position has to be 
            // calibrated.
        ...
        else if( notif_value_received == SP_HOME )
        {
            ...
            // cart_position_setpoint_cm_cli low-pass filter. Low-pass filter is
            // used for both setpoint sources to smooth out discontinous input
...
void set_output_voltage_nobounce( float voltage )
{
    // Not elegant way of doing debouncing but for now it's alright
