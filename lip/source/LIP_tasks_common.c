...
// =============================================================================
// Holds data from ADC3 tranfered over DMA, init code is in 
// motor_driver.c/dcm_init()
...
// Cart position calibrated flag indicated that cart has real position match 
// cart sensor reading 
//     1 - calibrated
//     0 - not calibrated
...
// Used as global flag to indicate that lookup_index in swingup task should be
// reset to zero. This have to be done on each call to cli command "swingup"
uint32_t reset_lookup_index = 0;

// Global flag to signal that swingup task has to be reset. Reset meaning start
// swingup procedure from the very begining, it doesn't reset the task itself.
...
// Reset meaning start "home" command procedure from the very begining, it 
// doesn't reset the task itself. Should be set to 1 with every call to cli 
// command "swingup"
uint32_t reset_home = 0;

// Global flag to signal that test command should be reset.
// Reset meaning start "test" command procedure from the very begining, it 
// doesn't reset the task itself
// uint32_t reset_test = 0;

// These are used as global variables to hold four control signal components
// from active controller task. COM_SEND_CTRL_DEBUG is defined in main_LIP.c
...
void LIP_create_Tasks()
{
    ...
    util_task_handle = xTaskCreateStatic( 
        util_task,
        (const char*) "Util",
        UTIL_STACK_DEPTH,
        (void *) 0,
    ...
    swingup_task_handle = xTaskCreateStatic( 
        swingup_task,
        ( const char * ) "Swingup", 
        SWINGUP_STACK_DEPTH,
        ( void * ) 0,
