...
void ctrl_5_FSF_uppos_task( void *pvParameters )
{
    ...
    for ( ;; ) {
        ...
        // angle range from swingup to upc, set up if watchdog task
        uint8_t is_pend_angle_ok = 
            switch_angle_low < pendulum_angle_in_base_range_upc &&
        ...
        if ( is_pend_angle_ok ) {
            ...
            pend_position_error = 
                pendulum_arm_angle_setpoint_rad_upc - pend_angle[ 0 ];
            pend_speed_error = - pend_speed[ 0 ];

            // Calculate control signal contribution of each state variable
            // error Non linear cart position gain. When cart postion 
            // error is >0 linear feedback with offset +1V is used to 
            // compensate for voltage deadzone, for <0 error, y-axis mirror
            //  is used. graph: https://www.desmos.com/calculator/ycgnqpyy9y

            // Default cart position error gain is gains[0]
            if( cart_position_error > 0.0f ) {
                // ctrl_cart_position_error =   
                //     tanhf( 7.0f * cart_position_error ) * 
